...
import { getBatches } from "../../apiCalls/batchCall.js";
...
const CreateAssignment = () => {
  ...
  const [batchId, setBatchId] = useState(""); // selected batch id
  const [questions, setQuestions] = useState([emptyQuestion()]);
  const [loading, setLoading] = useState(false);
  const [msg, setMsg] = useState(null);
  const navigate = useNavigate();

  // batches state for dropdown
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setBatchesError(err?.message || JSON.stringify(err));
      } finally {
    ...
    setQuestions((prev) =>
      prev.map((q, i) => {
        ...
        if (field === "correctOption")
          return { ...q, correctOption: Number(value) };
        return q;
      })
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMsg(err?.message || JSON.stringify(err) || "Error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {batchesLoading ? (
            <div className="p-2">Loading batches...</div>
          ) : batchesError ? (
            <div className="p-2 text-red-600">
              Failed to load batches: {batchesError}
            </div>
          ) : (
            <select
              value={batchId}
            ...
            >
              ...
              {batches.map((b) => (
                <option key={b._id} value={b._id}>
                  {b.name} {b.students ? `(${b.students.length} students)` : ""}
                </option>
              ))}
            </select>
          )}
          <p className="text-xs text-gray-500 mt-1">
            Only batches that belong to your account are shown.
          </p>
        </div>
